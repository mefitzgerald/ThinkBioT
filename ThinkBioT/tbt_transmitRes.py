...
class Tbt_RbTX (rockBlockProtocol):
    ...
    def txUpdatesystem():
        ...
        f.write("Sucessfully transmitted data : " + str(datetime.datetime.now()) + "\n")
        f.close()
    ...
    def main(self):
        ...
        msg = str(epochtime) +"," + tx_aci +"," + tx_adi + "," + tx_aei + "," + tx_bi + "," + tx_ndsi + "," + str(tx_speciesCount) + "," + str(tx_samplesCount) + "," + str(tx_alertCount) + ",,,,"
        print(msg)
        
        # rb.sendMessage(msg)
        # rb.close()
        txUpdatesystem(tx_sessionID)
        c.close()
